...
const SUP_COLOR = PILLAR_META.supplements.color;
const SUP_BG = PILLAR_META.supplements.bgLight;
...
function AddItemForm({ section, onAdd, onCancel }: AddItemFormProps) {
  ...
  const handleSubmit = () => {
    ...
    onAdd({
      ...
      timeOfDay: section === "morning" ? "morning" : "night",
    ...
    <div className="bg-[#fbfbfd] border border-[#d2d2d7]/30 rounded-2xl p-5 space-y-4 animate-slide-up">
      ...
      <div>
        ...
        <div className="relative">
          <button
            ...
            className="w-11 h-11 rounded-xl bg-white border border-[#d2d2d7]/40 flex items-center justify-center text-[20px] hover:border-[#ff9f0a]/40 transition-apple"
          >
            {icon}
          </button>
          {showEmojiPicker && (
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#d2d2d7]/40 text-[15px] text-[#1d1d1f] placeholder:text-[#d2d2d7] focus:border-[#ff9f0a]/50 focus:ring-1 focus:ring-[#ff9f0a]/20 outline-none transition-apple"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="Dosage and purpose"
          className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#d2d2d7]/40 text-[15px] text-[#1d1d1f] placeholder:text-[#d2d2d7] focus:border-[#ff9f0a]/50 focus:ring-1 focus:ring-[#ff9f0a]/20 outline-none transition-apple"
        />
      </div>
      ...
      <div>
        <label className="text-[11px] font-medium text-[#86868b] uppercase tracking-wide block mb-1.5">
          Timing (
          {section === "morning" ? "minutes after wake" : "minutes before bed"})
        </label>
        <div className="flex items-center gap-3">
          <input
            ...
            min={0}
            max={720}
            className="w-24 px-3.5 py-2.5 rounded-xl bg-white border border-[#d2d2d7]/40 text-[15px] text-[#1d1d1f] focus:border-[#ff9f0a]/50 focus:ring-1 focus:ring-[#ff9f0a]/20 outline-none transition-apple"
          />
      ...
      <button
        ...
        style={{
          backgroundColor: name.trim() ? SUP_COLOR : "#e5e5ea",
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Enabled/disabled badge for non-edit mode */}
          <div className="flex flex-wrap gap-1.5 mt-2">
            {!item.isEnabled && (
              <span className="text-[10px] font-medium bg-[#f5f5f7] text-[#86868b] rounded-full px-2 py-0.5">
                Disabled
              </span>
            )}
            {item.recommendedBy && item.recommendedBy.map((source) => (
              <span
                key={source}
        ...
        {isEditMode ? (
          <div className="flex items-center gap-2 shrink-0 pt-1">
            <button
              ...
              style={{
                backgroundColor: item.isEnabled ? SUP_COLOR : undefined,
              }}
      ...
      {isExpanded && !isEditMode && (
        <div className="px-4 pb-4 pt-0 space-y-3 animate-slide-up">
          ...
          {item.evidenceNote && (
            <div className="rounded-xl p-3.5" style={{ backgroundColor: SUP_BG }}>
              <p className="text-[11px] font-semibold uppercase tracking-wide mb-1.5" style={{ color: SUP_COLOR }}>Evidence</p>
              <p className="text-[13px] text-[#1d1d1f] leading-relaxed">{item.evidenceNote}</p>
            </div>
          )}

          {/* Disclaimer */}
          <div className="rounded-xl p-3 bg-[#fff8f0] border border-[#ff9f0a]/10">
            <p className="text-[11px] text-[#86868b] leading-relaxed">
              ⚠️ Supplements are not FDA-regulated. Always consult your physician before starting any new supplement, especially if you take medications.
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Brand, dosage, when you take it..."
              rows={2}
              className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-[#d2d2d7]/30 text-[13px] text-[#1d1d1f] placeholder:text-[#d2d2d7] resize-none focus:border-[#ff9f0a]/50 focus:ring-1 focus:ring-[#ff9f0a]/20 outline-none transition-apple"
            />
          </div>
        </div>
      )}
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2.5">
          <span className="text-[20px]">{isMorning ? "🌅" : "🌙"}</span>
          <h2 className="text-[17px] font-semibold text-[#1d1d1f]">
            {isMorning ? "Morning Stack" : "Evening Stack"}
          </h2>
        </div>
        ...
        <div className="pt-1">
          {showAddForm ? (
            ...
            <button
              ...
              className="w-full py-3 rounded-2xl border-2 border-dashed border-[#d2d2d7]/40 text-[13px] font-semibold text-[#86868b] hover:border-[#ff9f0a]/30 hover:text-[#ff9f0a] transition-apple"
            >
              + Add Custom Supplement
            </button>
          )}
...
export default function SupplementsPage() {
  ...
  const supplementItems = state.protocols.supplements;

  const { morningItems, eveningItems } = useMemo(() => {
    const withTimes = supplementItems.map((item) => ({
      ...item,
  ...
  }, [supplementItems, state.settings]);
  ...
  const handleToggleEnabled = useCallback(
    (id: string) => {
      const updated = supplementItems.map((item) =>
        item.id === id ? { ...item, isEnabled: !item.isEnabled } : item
      );
      updateProtocols("supplements", updated);
    },
    [supplementItems, updateProtocols]
  );

  const handleDelete = useCallback(
    (id: string) => {
      const updated = supplementItems.filter((item) => item.id !== id);
      updateProtocols("supplements", updated);
    },
    [supplementItems, updateProtocols]
  );
  ...
  const handleAddItem = useCallback(
    (itemData: Omit<ProtocolItem, "id" | "createdAt" | "sortOrder">) => {
      const newItem: ProtocolItem = {
        ...itemData,
        id: `supplements-custom-${Date.now()}`,
        sortOrder: supplementItems.length + 1,
        createdAt: new Date().toISOString(),
      };
      updateProtocols("supplements", [...supplementItems, newItem]);
    },
    [supplementItems, updateProtocols]
  );
  ...
  const enabledCount = supplementItems.filter((i) => i.isEnabled).length;
    ...
    <Shell>
      <div className="space-y-8 pb-4">
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[15px] text-[#86868b] mt-1">
                Evidence-based supplementation stacks
              </p>
            </div>
            <button
              ...
              style={isEditMode ? { backgroundColor: SUP_COLOR } : undefined}
            >
          ...
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ backgroundColor: SUP_BG, color: SUP_COLOR }}
            >
              <span className="text-[15px]">💊</span>
              {enabledCount} active
            </div>
            <span className="text-[13px] text-[#86868b]">
              {totalCount - enabledCount} optional available
            </span>
          </div>

          {/* Disclaimer banner */}
          <div className="bg-[#fff8f0] border border-[#ff9f0a]/15 rounded-2xl p-4">
            <div className="flex items-start gap-3">
              <span className="text-[18px] shrink-0">⚠️</span>
              <p className="text-[12px] text-[#86868b] leading-relaxed">
                Supplements are not a substitute for a nutrient-dense diet.
        ...
        <div className="bg-gradient-to-br from-[#fbfbfd] to-[#f5f5f7] border border-[#d2d2d7]/20 rounded-2xl p-5">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-[16px]" style={{ backgroundColor: SUP_BG }}>
              ✨
            </div>
            <div>
              <p className="text-[13px] font-semibold mb-1" style={{ color: SUP_COLOR }}>AI Insight</p>
              <p className="text-[13px] text-[#1d1d1f] leading-relaxed">
                The Huberman sleep stack (magnesium threonate + L-theanine +
                apigenin) is one of the most widely discussed supplement
                protocols. Start with magnesium alone, then add others one at a
                time to assess individual response.
              </p>
            </div>
          </div>
        </div>
