...
function recoveryOf(log?: DailyLog): number {
  if (!log) return 0;
  const e = ((log.energyLevel ?? 0) / 5) * 100;
  const m = ((log.moodLevel ?? 0) / 5) * 100;
  const s = log.pillarScores?.sleep ?? 0;
...
export default function RecoveryPage() {
  ...
  const drivers = useMemo(() => {
    const sleep = log.pillarScores?.sleep ?? 0;
    const energy = ((log.energyLevel ?? 0) / 5) * 100;
    const mood = ((log.moodLevel ?? 0) / 5) * 100;
    return [
      { label: "Sleep Quality", value: sleep, color: "var(--sleep)" },
      { label: "Energy", value: energy, color: "var(--readiness)" },
      { label: "Mood", value: mood, color: "var(--vitality)" },
    ];
  }, [log]);

  const guidance = useMemo(() => {
    if (recovery === 0)
      return "Log your energy and mood below to generate today's recovery reading.";
    if (recovery >= 75)
      return "Fully recovered. Your system can handle high strain today — train hard, think deeply, push your edges.";
    if (recovery >= 50)
      return "Moderately recovered. Maintain your normal load, but stay attentive to fatigue signals.";
    return "Recovery is compromised. Prioritize rest, hydration, and an early night. Keep training light and restorative.";
  }, [recovery]);
    ...
    <Shell>
      <div className="flex flex-col gap-7">
        ...
        <div className="anim-rise d1 flex flex-col items-center">
          <RingScore
            value={recovery}
            color={scoreColor(recovery)}
            label={scoreWord(recovery)}
            sublabel="Recovery"
          />
        </div>
          ...
          <div className="mt-5 space-y-6">
            <div>
              ...
              <div className="flex justify-between gap-2">
                {[1, 2, 3, 4, 5].map((lv) => {
                  const on =
                    log.energyLevel !== null && lv <= (log.energyLevel ?? 0);
                  return (
                    <button
                      key={lv}
                      onClick={() =>
                      ...
                      className="press tr-fast flex-1 rounded-[var(--r-sm)] py-3.5 text-[18px]"
                      style={{
                        background: on
                          ? "var(--readiness-soft)"
                          : "var(--surface-2)",
            ...
            <div>
              ...
              <div className="flex justify-between gap-2">
                {["😞", "😕", "😐", "🙂", "😊"].map((f, i) => {
                  const on =
                    log.moodLevel !== null && i + 1 <= (log.moodLevel ?? 0);
                  return (
                    <button
                      key={i}
                      onClick={() =>
                      ...
                      className="press tr-fast flex-1 rounded-[var(--r-sm)] py-3 text-[20px]"
                      style={{
                        background: on
                          ? "var(--vitality-soft)"
                          : "var(--surface-2)",
